...
> **Ghi chú bài cá nhân:** Với các câu hỏi cần tổng hợp nhiều ý, khó tìm evidence hoặc cần bao phủ nhiều nguồn, có thể tăng `top_k` lên khoảng `50` (vì chạy local không dùng jina nên cần top k cao hơn cho chắc chắn) để retriever lấy đủ ngữ cảnh hơn. Tuy nhiên, nếu câu hỏi không có câu trả lời đúng trong data, `top_k` quá cao có thể kéo thêm nhiều context nhiễu và làm LLM dễ hallucinate. Với các câu hỏi kiểu này, nên giảm `top_k` xuống thấp hơn và yêu cầu model trả lời “I cannot verify this information” khi không có evidence rõ ràng.
